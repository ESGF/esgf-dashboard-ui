...
/*    $http.get("/esgf-dashboard-ui/getGeoDataNodes", {params: {project: "all"}}).success(
	...
	$scope.exportContinent = function () {
		...
		window.open('/esgf-dashboard-ui/downloadCSVbyContinent?project=' + project + '&datanode=all');
	}
	
	$scope.exportCountry = function () {
		var project = $scope.slctItem1.name
		window.open('/esgf-dashboard-ui/downloadCSVbyCountry?project=' + project + '&datanode=all');
	}
