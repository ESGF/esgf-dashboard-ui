...
public enum SqlQuery {
    ...
    // cmip6 all variables
    CMIP6_ALL_VARIABLES_NUMBER_DOWNLOADS_ALL("SELECT SUM(number_of_downloads) AS measure, variable_code AS dimension, variable_long_name FROM cmip6_dmart_variable_host_time WHERE year>=? GROUP BY variable_code, variable_long_name ORDER BY measure DESC LIMIT 100;"),
    CMIP6_ALL_VARIABLES_DOWNLOADED_DATA_ALL("SELECT SUM(total_size)/1024/1024/1024 AS measure, variable_code AS dimension, cf_standard_name, variable_long_name FROM cmip6_dmart_variable_host_time WHERE year>=? GROUP BY variable_code, cf_standard_name, variable_long_name ORDER BY measure DESC LIMIT 100;"),
    
    CMIP6_ALL_VARIABLES_NUMBER_DOWNLOADS("SELECT SUM(number_of_downloads) AS measure, variable_code AS dimension, variable_long_name FROM cmip6_dmart_variable_host_time WHERE year>=? AND host_name=? GROUP BY variable_code, variable_long_name ORDER BY measure DESC LIMIT 100;"),
    CMIP6_ALL_VARIABLES_DOWNLOADED_DATA("SELECT SUM(total_size)/1024/1024/1024 AS measure, variable_code AS dimension, variable_long_name FROM cmip6_dmart_variable_host_time WHERE year>=? AND host_name=? GROUP BY variable_code, variable_long_name ORDER BY measure DESC LIMIT 100;"),
    ...
    // cmip6 simple chart
    CMIP6_NUMBER_DOWNLOADS_BY_EXPERIMENT_ID_ALL("SELECT SUM(number_of_downloads) AS measure, experiment_id AS dimension FROM cmip6_dmart_experiment_host_time WHERE year>=? GROUP BY experiment_id;"),
    CMIP6_NUMBER_DOWNLOADS_BY_SOURCE_ID_ALL("SELECT SUM(number_of_downloads) AS measure, source_id AS dimension FROM cmip6_dmart_source_id_host_time WHERE year>=? GROUP BY source_id;"),
    CMIP6_DOWNLOADED_DATA_BY_EXPERIMENT_ID_ALL("SELECT SUM(total_size)/1024/1024/1024 AS measure, experiment_id AS dimension FROM cmip6_dmart_experiment_host_time WHERE year>=? GROUP BY experiment_id;"),
    CMIP6_DOWNLOADED_DATA_BY_SOURCE_ID_ALL("SELECT SUM(total_size)/1024/1024/1024 AS measure, source_id AS dimension FROM cmip6_dmart_source_id_host_time WHERE year>=? GROUP BY source_id;"),
    
    CMIP6_NUMBER_DOWNLOADS_BY_EXPERIMENT_ID("SELECT SUM(number_of_downloads) AS measure, experiment_id AS dimension FROM cmip6_dmart_experiment_host_time WHERE year>=? AND host_name=? GROUP BY experiment_id;"),
    CMIP6_NUMBER_DOWNLOADS_BY_SOURCE_ID("SELECT SUM(number_of_downloads) AS measure, source_id AS dimension FROM cmip6_dmart_source_id_host_time WHERE year>=? AND host_name=? GROUP BY source_id;"),
    CMIP6_DOWNLOADED_DATA_BY_EXPERIMENT_ID("SELECT SUM(total_size)/1024/1024/1024 AS measure, experiment_id AS dimension FROM cmip6_dmart_experiment_host_time WHERE year>=? AND host_name=? GROUP BY experiment_id;"),
    CMIP6_DOWNLOADED_DATA_BY_SOURCE_ID("SELECT SUM(total_size)/1024/1024/1024 AS measure, source_id AS dimension FROM cmip6_dmart_source_id_host_time WHERE year>=? AND host_name=? GROUP BY source_id;"),
    ...
    // cmip5 all variables
    ALL_VARIABLES_NUMBER_DOWNLOADS_ALL("SELECT SUM(number_of_downloads) AS measure, variable_code AS dimension, cf_standard_name, variable_long_name FROM cmip5_dmart_variable_host_time WHERE year>=? GROUP BY variable_code, cf_standard_name, variable_long_name ORDER BY measure DESC LIMIT 100;"),
    ALL_VARIABLES_DOWNLOADED_DATA_ALL("SELECT SUM(total_size)/1024/1024/1024 AS measure, variable_code AS dimension, cf_standard_name, variable_long_name FROM cmip5_dmart_variable_host_time WHERE year>=? GROUP BY variable_code, cf_standard_name, variable_long_name ORDER BY measure DESC LIMIT 100;"),
    
    ALL_VARIABLES_NUMBER_DOWNLOADS("SELECT SUM(number_of_downloads) AS measure, variable_code AS dimension, cf_standard_name, variable_long_name FROM cmip5_dmart_variable_host_time WHERE year>=? AND host_name=? GROUP BY variable_code, cf_standard_name, variable_long_name ORDER BY measure DESC LIMIT 100;"),
    ALL_VARIABLES_DOWNLOADED_DATA("SELECT SUM(total_size)/1024/1024/1024 AS measure, variable_code AS dimension, cf_standard_name, variable_long_name FROM cmip5_dmart_variable_host_time WHERE year>=? AND host_name=? GROUP BY variable_code, cf_standard_name, variable_long_name ORDER BY measure DESC LIMIT 100;"),

    // cmip5 top twenty variable
    ...
    // cmip5 simple chart
    CMIP5_NUMBER_DOWNLOADS_BY_EXPERIMENT_ALL("SELECT SUM(number_of_downloads) AS measure, experiment_name AS dimension FROM cmip5_dmart_experiment_host_time WHERE year>=? GROUP BY experiment_name;"),
    CMIP5_NUMBER_DOWNLOADS_BY_MODEL_ALL("SELECT SUM(number_of_downloads) AS measure, model_name AS dimension FROM cmip5_dmart_model_host_time WHERE year>=? GROUP BY model_name;"),
    CMIP5_DOWNLOADED_DATA_BY_EXPERIMENT_ALL("SELECT SUM(total_size)/1024/1024/1024 AS measure, experiment_name AS dimension FROM cmip5_dmart_experiment_host_time WHERE year>=? GROUP BY experiment_name;"),    
    CMIP5_DOWNLOADED_DATA_BY_MODEL_ALL("SELECT SUM(total_size)/1024/1024/1024 AS measure, model_name AS dimension FROM cmip5_dmart_model_host_time WHERE year>=? GROUP BY model_name;"),
    
    CMIP5_NUMBER_DOWNLOADS_BY_EXPERIMENT("SELECT SUM(number_of_downloads) AS measure, experiment_name AS dimension FROM cmip5_dmart_experiment_host_time WHERE year>=? AND host_name=? GROUP BY experiment_name;"),
    CMIP5_NUMBER_DOWNLOADS_BY_MODEL("SELECT SUM(number_of_downloads) AS measure, model_name AS dimension FROM cmip5_dmart_model_host_time WHERE year>=? host_name=? GROUP BY model_name;"),
    CMIP5_DOWNLOADED_DATA_BY_EXPERIMENT("SELECT SUM(total_size)/1024/1024/1024 AS measure, experiment_name AS dimension FROM cmip5_dmart_experiment_host_time WHERE year>=? AND host_name=? GROUP BY experiment_name;"),    
    CMIP5_DOWNLOADED_DATA_BY_MODEL("SELECT SUM(total_size)/1024/1024/1024 AS measure, model_name AS dimension FROM cmip5_dmart_model_host_time WHERE year>=? AND host_name=? GROUP BY model_name;"),
    ...
    CORDEX_ALL_VARIABLES_NUMBER_DOWNLOADS_ALL("SELECT SUM(number_of_downloads) AS measure, variable_code AS dimension, cf_standard_name, variable_long_name FROM cordex_dmart_variable_host_time WHERE year>=? GROUP BY variable_code, cf_standard_name, variable_long_name ORDER BY measure DESC LIMIT 100;"),
    CORDEX_ALL_VARIABLES_DOWNLOADED_DATA_ALL("SELECT SUM(total_size)/1024/1024/1024 AS measure, variable_code AS dimension, cf_standard_name, variable_long_name FROM cordex_dmart_variable_host_time WHERE year>=? GROUP BY variable_code, cf_standard_name, variable_long_name ORDER BY measure DESC LIMIT 100;"),
    
    CORDEX_ALL_VARIABLES_NUMBER_DOWNLOADS("SELECT SUM(number_of_downloads) AS measure, variable_code AS dimension, cf_standard_name, variable_long_name FROM cordex_dmart_variable_host_time WHERE year>=? AND host_name=? GROUP BY variable_code, cf_standard_name, variable_long_name ORDER BY measure DESC LIMIT 100;"),
    CORDEX_ALL_VARIABLES_DOWNLOADED_DATA("SELECT SUM(total_size)/1024/1024/1024 AS measure, variable_code AS dimension, cf_standard_name, variable_long_name FROM cordex_dmart_variable_host_time WHERE year>=? AND host_name=? GROUP BY variable_code, cf_standard_name, variable_long_name ORDER BY measure DESC LIMIT 100;"),
    ...
    CORDEX_NUMBER_DOWNLOADS_BY_DOMAIN("SELECT SUM(number_of_downloads) AS measure, domain AS dimension FROM cordex_dmart_domain_host_time WHERE year>=? AND host_name=? GROUP BY domain;"),
    CORDEX_DOWNLOADED_DATA_BY_DOMAIN("SELECT SUM(total_size)/1024/1024/1024 AS measure, domain AS dimension FROM cordex_dmart_domain_host_time WHERE year>=? AND host_name=? GROUP BY domain;"),
    
    CORDEX_NUMBER_DOWNLOADS_BY_DRIVING_MODEL_ALL("SELECT SUM(number_of_downloads) AS measure, driving_model AS dimension FROM cordex_dmart_driving_model_host_time WHERE year>=? GROUP BY driving_model;"),
    CORDEX_DOWNLOADED_DATA_BY_DRIVING_MODEL_ALL("SELECT SUM(total_size)/1024/1024/1024 AS measure, driving_model AS dimension FROM cordex_dmart_driving_model_host_time WHERE year>=? GROUP BY driving_model;"),
    
    CORDEX_NUMBER_DOWNLOADS_BY_DRIVING_MODEL("SELECT SUM(number_of_downloads) AS measure, driving_model AS dimension FROM cordex_dmart_driving_model_host_time WHERE year>=? AND host_name=?GROUP BY driving_model;"),
    CORDEX_DOWNLOADED_DATA_BY_DRIVING_MODEL("SELECT SUM(total_size)/1024/1024/1024 AS measure, driving_model AS dimension FROM cordex_dmart_driving_model_host_time WHERE year>=? AND host_name=?GROUP BY driving_model;"),
    
    CORDEX_NUMBER_DOWNLOADS_BY_RCM_ALL("SELECT SUM(number_of_downloads) AS measure, rcm_name AS dimension FROM cordex_dmart_rcm_host_time WHERE year>=? GROUP BY rcm_name;"),
    CORDEX_DOWNLOADED_DATA_BY_RCM_ALL("SELECT SUM(total_size)/1024/1024/1024 AS measure, rcm_name AS dimension FROM cordex_dmart_rcm_host_time WHERE year>=? GROUP BY rcm_name;"),
    
    CORDEX_NUMBER_DOWNLOADS_BY_RCM("SELECT SUM(number_of_downloads) AS measure, rcm_name AS dimension FROM cordex_dmart_rcm_host_time WHERE year>=? AND host_name=? GROUP BY rcm_name;"),
    CORDEX_DOWNLOADED_DATA_BY_RCM("SELECT SUM(total_size)/1024/1024/1024 AS measure, rcm_name AS dimension FROM cordex_dmart_rcm_host_time WHERE year>=? AND host_name=? GROUP BY rcm_name;"),
 ...
 // obs4mips simple chart
    ...
    OBS4MIPS_NUMBER_DOWNLOADS_BY_REALM_ALL("SELECT SUM(number_of_downloads) AS measure, realm AS dimension FROM obs4mips_dmart_realm_host_time WHERE year>=? GROUP BY realm;"),
    OBS4MIPS_NUMBER_DOWNLOADS_BY_SOURCE_ALL("SELECT SUM(number_of_downloads) AS measure, source_id AS dimension FROM obs4mips_dmart_source_host_time WHERE year>=? GROUP BY source_id;"),
    ...
    OBS4MIPS_DOWNLOADED_DATA_BY_REALM_ALL("SELECT SUM(total_size)/1024/1024/1024 AS measure, realm AS dimension FROM obs4mips_dmart_realm_host_time WHERE year>=? GROUP BY realm;"),
    OBS4MIPS_DOWNLOADED_DATA_BY_SOURCE_ALL("SELECT SUM(total_size)/1024/1024/1024 AS measure, source_id AS dimension FROM obs4mips_dmart_source_host_time WHERE year>=? GROUP BY source_id;"),
    ...
    OBS4MIPS_NUMBER_DOWNLOADS_BY_REALM("SELECT SUM(number_of_downloads) AS measure, realm AS dimension FROM obs4mips_dmart_realm_host_time WHERE year>=? AND host_name=? GROUP BY realm;"),
    OBS4MIPS_NUMBER_DOWNLOADS_BY_SOURCE("SELECT SUM(number_of_downloads) AS measure, source_id AS dimension FROM obs4mips_dmart_source_host_time WHERE year>=? AND host_name=? GROUP BY source_id;"),
    ...
    OBS4MIPS_DOWNLOADED_DATA_BY_REALM("SELECT SUM(total_size)/1024/1024/1024 AS measure, realm AS dimension FROM obs4mips_dmart_realm_host_time WHERE year>=? AND host_name=? GROUP BY realm;"),
    OBS4MIPS_DOWNLOADED_DATA_BY_SOURCE("SELECT SUM(total_size)/1024/1024/1024 AS measure, source_id AS dimension FROM obs4mips_dmart_source_host_time WHERE year>=? AND host_name=? GROUP BY source_id;");
